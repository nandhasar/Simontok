...
## 📋 Cara Pakai di `assistant.html`

### 1. Include di `<head>`

```html
<!-- Pastikan ai-client.js di-load SEBELUM assistant script -->
<script src="js/ai-client.js"></script>
```

### 2. Ganti fungsi `callAI()` di `assistant.html`

```javascript
// SEBELUM (versi inline di assistant.html):
function callAI(apiKey, messages) {
  return fetch('https://openrouter.ai/...',  { ... });
}

// SESUDAH — cukup 1 baris:
function callAI(messages) {
  return AI_CLIENT.sendChat(messages)
    .then(function(res) { return res.text; });
}
```

### 3. Cek API key sebelum kirim
...
### 4. Override model (opsional)

```javascript
// Pakai model lain untuk satu request tertentu
AI_CLIENT.sendChat(messages, { model: 'openai/gpt-4o-mini' });

// Atau global — ubah variabel di config:
...
| Fitur | Detail |
|-------|--------|
| **Provider** | OpenRouter saja, tidak ada fallback |
| **Model Default** | `google/gemma-4-31b-it` (baca dari `AI_MODEL` global) |
| **Timeout** | 30 detik via `AbortController` |
| **Error Handling** | HTTP 400/401/402/429/500/502/503 → pesan ramah Bahasa Indonesia |
| **API Key** | Baca dari `sessionStorage('simontok-apikey')` — diset saat login |
| **Stats** | `getStats()` → request count, total token, latency, error log |
| **Clean Messages** | Strip field `timestamp` dll sebelum kirim ke API |
| **Pola Module** | IIFE `var AI_CLIENT = (function(){...})()` — tidak polusi global |
